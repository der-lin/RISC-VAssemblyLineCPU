module testbench();
  ...
  // check results based on test37.s specification
  always @(negedge clk) begin
    if (~reset) begin
      cycle_count = cycle_count + 1;
      
      // 1. 超时检测：如果仿真超过 3000 个周期还没结束，说明陷入了错误的死循环
      if (cycle_count > 3000) begin
        $display("TIMEOUT FAILED: The program is stuck in an unexpected loop.");
        $stop;
      end

      // 2. 终点检测：PC 到达 0x24C (END: beq x0, x0, END)
      if (dut.rv.PC === 32'h0000024C) begin
        // 等待几个周期，让最后几条指令完全写回寄存器
        #20; 
        
        // 3. 读取 x31 寄存器的值进行校验 (层次引用: dut -> rv -> rf -> rf[31])
        if (dut.rv.rf.rf[31] === 32'd507) begin
          $display("x31 accumulator PASSED. Simulation succeeded!");
        end else begin
          $display("x31 accumulator FAILED: expected 507, got %d", dut.rv.rf.rf[31]);
        end
        $stop;
      end
    end
