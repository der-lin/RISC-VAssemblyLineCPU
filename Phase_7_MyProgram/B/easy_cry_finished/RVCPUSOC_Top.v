...
module RVCPUSOC_Top(
  ...
  // instantiation of cpu
   riscvpipeline U_CPU(
         ...
         .reset(rst),               // reset
         .reg_sel(sw_i[4:0]),       // SW[4:0] selects register x0~x31 when SW[5]=1
         .reg_data(reg_data),       // selected register value for display channel 1xxxxx
         .PC(pc),                   // PC
         .Instr(instr),             // instruction
         .MemWrite(memwrite),       // memory/IO(seg7) write signal
         .DataAdr(cpu_data_addr),   // address from cpu to memory/IO(seg7)
  ...
  // instantiation of data memory  
   dm    U_DM(
         .clk(Clk_CPU),   // cpu clock
         .we(ram_we),   // ram write
         .a(ram_addr), // ram address
         .addr_lsb(cpu_data_addr[1:0]), // byte offset for sb/sh
         .funct3(funct3M), // store width selector
         .wd(dm_din),    // data to ram
         .rd(dm_dout)   // data from ram
         );
  ...
  // instantiation of Multi_CH32
   Multi_CH32 U_Multi (
          ...
          .ctrl(sw_i[5:0]),                 // SW[5:0], keep 000000 for application display; SW[9:6] is used by software as digit input
          ...
          .data7({22'b0, ram_addr, 2'b00}), // test channel 7--ram address
